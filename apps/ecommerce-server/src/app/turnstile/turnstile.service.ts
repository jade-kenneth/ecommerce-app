...
export class TurnstileService {
  ...
  private getSecretKey() {
    const secretKey = this.config.getString(
  ...
  private getToken(request: TurnstileRequest) {
    const candidates = [
      ...
      this.getNestedBodyValue(request.body, ['variables', 'cfTurnstileResponse']),
      this.getNestedBodyValue(request.body, ['variables', 'cf-turnstile-response']),
      this.getNestedBodyValue(request.body, ['variables', 'input', 'turnstileToken']),
      this.getNestedBodyValue(request.body, ['variables', 'input', 'cfTurnstileResponse']),
      this.getNestedBodyValue(request.body, ['variables', 'input', 'cf-turnstile-response']),
    ];
  ...
  private getRemoteIp(request: TurnstileRequest) {
    const forwardedFor = this.toTrimmedString(request.headers['x-forwarded-for']);
    if (forwardedFor) {
  ...
  private async verifyToken(secretKey: string, token: string, remoteIp?: string) {
  ...
  private parseSiteverifyResponse(payload: unknown): TurnstileSiteverifyResponse {
